...
const createLambdaEdgeContext = require("./createLambdaEdgeContext");
...
class Offline {
    ...
    this.commands = {
      offline: {
        ...
        options: {
          ...
          exec: {
            usage:
              "When provided, a shell script is executed when the server starts up, and the server will shut domn after handling this command."
          },
          noAuth: {
  ...
  _setOptions() {
    ...
    this.globalBabelOptions = ((this.service.custom || {})[
      "serverless-offline"
    ] || {}).babelOptions;
  ...
  _createRoutes() {
    ...
    if (
      ["nodejs", "nodejs4.3", "nodejs6.10", "babel"].indexOf(serviceRuntime) ===
      -1
    ) {
      this.printBlankLine();
      this.serverlessLog(
    ...
    Object.keys(this.service.functions).forEach(key => {
      ...
      ((fun.events && fun.events.length) || this.serverlessLog("(none)")) &&
        fun.events.forEach(event => {
          ...
          const integration =
            this.options.integration || endpoint.integration || "lambda-proxy";
    ...
    if (endpoint.authorizer) {
      ...
      if (typeof endpoint.authorizer === "object") {
        ...
        if (endpoint.authorizer.arn) {
          this.serverlessLog(
            `WARNING: Serverless Offline does not support non local authorizers: ${endpoint
              .authorizer.arn}`
          );
  ...
  _listen() {
    return new Promise((resolve, reject) => {
      this.server.start(err => {
        ...
        this.printBlankLine();
        this.serverlessLog(
          `Offline listening on http${this.options.httpsProtocol
            ? "s"
  ...
  _createResourceRoutes() {
    ...
    Object.keys(resourceRoutes).forEach(methodId => {
      ...
      if (`${method}`.toUpperCase() !== "GET") {
        return this.serverlessLog(
          `WARNING: ${method} proxy is not supported. Path '${pathResource}' is ignored.`
        );
      }
      if (!path) {
        return this.serverlessLog(
